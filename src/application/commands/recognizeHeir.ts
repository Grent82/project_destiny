...
export function recognizeHeir(state: GameState, heirId: string): GameState {
  ...
  const alreadyPending = next.pendingEvents.some((pe) => pe.eventId === 'event-heir-announcement')
  if (alreadyPending) return next

  const withEvent: GameState = {
    ...next,
    pendingEvents: [...next.pendingEvents, { eventId: 'event-heir-announcement', firedOnDay: next.day }],
  }
