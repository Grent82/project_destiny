...
describe('initializeRosterRelationships', () => {
  it('returns state unchanged when roster has fewer than 2 members', () => {
    const state = { ...initialStateWithIda, npcRuntimeStates: [initialStateWithIda.npcRuntimeStates[0]!] }
  ...
  it('creates both directed edges for each roster pair', () => {
    ...
    const [a, b] = [initialStateWithIda.npcRuntimeStates[0]!, initialStateWithIda.npcRuntimeStates[1]!]
  ...
  it('seeds Tier 1 authored bond (Ida ↔ Holst) when both on roster', () => {
    ...
    const state = {
      ...initialStateWithIda,
      npcRuntimeStates: [
        { ...initialStateWithIda.npcRuntimeStates[1]!, npcId: 'npc-ida-rhys' },
        holstEntry,
      ],
      relationships: {},
    }
