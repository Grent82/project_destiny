 ...
 *
 * Commands must go through these helpers instead of reaching into `state.npcRuntimeStates` directly, so that
 ...
 *
 * BRIDGE STATE (today): the only NpcRuntimeState persons live in `state.npcRuntimeStates`, and they are all
 ...
 *
 * NOTE (bridge): only returns NpcRuntimeState persons (currently `state.npcRuntimeStates`). It does NOT return
...
export function findNpc(state: GameState, npcId: string): NpcRuntimeState | undefined {
  return state.npcRuntimeStates.find((n) => n.npcId === npcId)
}
...
export function updateNpc(
  ...
  return {
    ...state,
    npcRuntimeStates: state.npcRuntimeStates.map((n) => (n.npcId === npcId ? updater(n) : n)),
  }
}
 ...
 * `state.npcRuntimeStates`; after C2/C3 returns the unified `state.npcRuntimeStates`.
 */
 ...
 * contract doc §2.1). Bridge: `state.npcRuntimeStates` already contains only player members, so the filter is
...
export function selectRosterNpcs(state: GameState): NpcRuntimeState[] {
  return state.npcRuntimeStates.filter((n) => n.playerRosterMember)
}
