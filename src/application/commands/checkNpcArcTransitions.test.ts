...
describe('checkNpcArcTransitions', () => {
  ...
  it('does not transition when minDaysInStage not met', () => {
    ...
    const state = { ...initialStateWithIda, npcRuntimeStates: [highTraitNpc] }

    const result = checkNpcArcTransitions(state)
    expect(result.npcRuntimeStates[0]!.npcArc!.stage).toBe('forming')
  })
  ...
  it('does not transition when trait condition not met', () => {
    ...
    const state = { ...initialStateWithIda, npcRuntimeStates: [lowTraitNpc] }
  ...
  it('advances forming → crystallizing when all conditions are met', () => {
    ...
    const result = checkNpcArcTransitions(state)
    expect(result.npcRuntimeStates[0]!.npcArc!.stage).toBe('crystallizing')
    expect(result.npcRuntimeStates[0]!.npcArc!.stageEnteredDay).toBe(state.day)
  })
  ...
  it('writes dominant trait type to stageFlags when entering crystallizing', () => {
    ...
    const state = { ...initialStateWithIda, npcRuntimeStates: [curiousNpc] }

    const result = checkNpcArcTransitions(state)
    expect(result.npcRuntimeStates[0]!.npcArc!.stageFlags['type-curious']).toBe(true)
  })
  ...
  it('writes discipline type when discipline is highest crystallization trait', () => {
    ...
    const state = { ...initialStateWithIda, npcRuntimeStates: [disciplinedNpc] }

    const result = checkNpcArcTransitions(state)
    expect(result.npcRuntimeStates[0]!.npcArc!.stageFlags['type-disciplined']).toBe(true)
  })
  ...
  it('advances crystallizing → set and queues type-specific + settled events', () => {
    ...
    const state = { ...initialStateWithIda, npcRuntimeStates: [highAllTraitNpc] }

    const result = checkNpcArcTransitions(state)
    expect(result.npcRuntimeStates[0]!.npcArc!.stage).toBe('set')
  ...
  it('does not transition from terminal stage (set)', () => {
    ...
    const state = { ...initialStateWithIda, npcRuntimeStates: [npc] }
...
describe('checkFracturedArcBranching', () => {
  function makeBrenNpc(stageEnteredDay: number, stage = 'cracking', flags: Record<string, boolean> = {}): NpcRuntimeState {
    return {
      ...
      traits: { ...initialStateWithIda.npcRuntimeStates[0]!.traits, loyalty: 68, empathy: 38, discipline: 81 },
    }
  }
  ...
  function makeAnchorNpc(): NpcRuntimeState {
    return {
      ...
      name: 'Anchor Person',
      traits: { ...initialStateWithIda.npcRuntimeStates[0]!.traits, empathy: 65 },
      npcArc: null,
    }
  }
  ...
  it('does nothing if Bren has less than 30 days in cracking stage', () => {
    ...
    const state = { ...initialStateWithIda, npcRuntimeStates: [bren] }
    const result = checkFracturedArcBranching(state)
    expect(result.npcRuntimeStates[0]!.npcArc!.stage).toBe('cracking')
  })
  ...
  it('advances to broken when no anchor NPC after 30 days', () => {
    ...
    const state = { ...initialStateWithIda, npcRuntimeStates: [bren] }
    const result = checkFracturedArcBranching(state)
    expect(result.npcRuntimeStates[0]!.npcArc!.stage).toBe('broken')
  })
  ...
  it('advances to healing when anchor NPC has empathy>60 and trust≥40', () => {
    ...
    const state: GameState = {
      ...initialStateWithIda,
      npcRuntimeStates: [anchor, bren],
      relationships: { [relKey]: { affinity: 50, trust: 45, respect: 40, fear: 0, loyalty: 30 } },
    }
    const result = checkFracturedArcBranching(state)
    const brenAfter = result.npcRuntimeStates.find((n) => n.npcId === bren.npcId)!
